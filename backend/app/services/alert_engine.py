...
async def process_location_for_alerts(
    *,
    ...
    students = await db.students.find({"stop_id": stop["stop_id"], "route_id": str(route["_id"])}).to_list(500)
    ...
    for parent in parents:
        ...
        await db.alert_logs.insert_one({**alert_filter, "sent_at": now, "type": "5_min_warning" if pref == 5 else "eta_warning"})
